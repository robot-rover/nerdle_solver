...
def _generate_entropies_gpu(guess_array, secret_array, in_dict):
    from nerdle_cuda import PythonClueContext
    num_guess = guess_array.shape[0]
    guess_packed = pack_array(guess_array, 15, dtype=np.uint64)
    num_secret = secret_array.shape[0]
    entropy_array = np.zeros((num_guess))
    entropies = {} if in_dict else []
    with PythonClueContext(num_guess, num_secret) as ctx:
        ctx.generate_entropies(guess_array, secret_array, entropy_array)
        generator = ((packed_eq, entropy) for packed_eq, entropy in zip(guess_packed, entropy_array))
        if in_dict:
